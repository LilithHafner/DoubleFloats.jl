...
(+)(x::Signed, y::DoubleFloat{T}) where {T<:IEEEFloat} = add_fpdb_db(T(x), y)
(+)(x::DoubleFloat{T}, y::Signed) where {T<:IEEEFloat} = add_dbfp_db(x, T(y))
...
(-)(x::Signed, y::DoubleFloat{T}) where {T<:IEEEFloat} = sub_fpdb_db(T(x), y)
(-)(x::DoubleFloat{T}, y::Signed) where {T<:IEEEFloat} = sub_dbfp_db(x, T(y))
